...
try {
  ...
  if(examBaseRaw!==null) {examBlocked=true;examWarning='Saved Hard 80 data could not be opened. It has not been changed. Download it before restoring a valid Exam 2 Complete Hard 80 backup.';}
...
async function commit(next,{restore=false}={}) {
  ...
  try {
    ...
    state=next;
    if(restore) {blocked=false;warning=memoryOnly?'Backup restored in memory. Download progress before closing.':'Exam 2 Complete Study backup restored. Earlier-version progress was not touched.';}
    return true;
...
function render() {
  ...
  root.innerHTML=`<a class="skip" href="#main">Skip to learning content</a><header class="site-header"><a class="brand" href="./" aria-label="NUR2460 Exam 2 Complete"><img src="icons/favicon.svg" width="38" height="38" alt=""><span>NUR2460 <small>Exam 2 · Complete private candidate</small></span></a><details class="legacy-links"><summary>Earlier versions</summary><a href="../">Original quiz ↗</a><a href="../study-checkpoint/">Study checkpoint ↗</a><p>Your earlier progress stays with its version.</p></details></header>
    ...
    ${mode==='study'&&state.history.length?reviewList(state.history,0):''}<p class="fine">Version ${bank.version} · Private integrated successor pending independent combined QA. No publication approval.</p></details>
...
function bind() {
  ...
  on('export','click',()=>download(JSON.stringify(mode==='hard80'?examState:state,null,2),mode==='hard80'?'NUR2460-complete-hard80-progress.json':'NUR2460-complete-study-progress.json'));
  on('raw-export','click',()=>{const key=mode==='hard80'?HARD80_SAVE_KEY:SAVE_KEY,raw=mode==='hard80'?examBaseRaw:baseRaw;try{download(localStorage.getItem(key)??raw??'null',mode==='hard80'?'NUR2460-complete-hard80-stored-recovery.json':'NUR2460-complete-study-stored-recovery.json');}catch{download(raw??'null',mode==='hard80'?'NUR2460-complete-hard80-stored-recovery.json':'NUR2460-complete-study-stored-recovery.json');}});
  ...
  on('backup-file','change',async event=>{
    ...
    try {
      ...
      if(mode==='hard80') {
        ...
        if(!confirm(`Restore this Hard 80 backup with ${answered} locked answers? This replaces only Exam 2 Complete Hard 80 progress.`))return;
        ...
        if(!confirm(`Restore ${candidate.history.length} submitted responses? This replaces only Exam 2 Complete Study progress. Earlier-version and Hard 80 progress are not affected.`))return;
